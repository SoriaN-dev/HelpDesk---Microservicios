...
public class TicketLifecycle {
    ...
    public void init(@Observes StartupEvent event, Vertx vertx) {
        System.out.println(">>>>>>TicketLifecycle.init()");
        ...
        try {
            ...
            var tags = List.of(
                    "traefik.enable=true",
                    "traefik.http.routers.ticket.rule=PathPrefix(`/app-ticket`)",
                    "traefik.http.middlewares.ticket-stripprefix.stripPrefix.prefixes=/app-ticket",
                    "traefik.http.routers.ticket.middlewares=ticket-stripprefix"
            );
            ...
            consuClient.registerService(serviceOptions)
                    .onSuccess(it->{
                        System.out.println("Ticket service registered in Consul");
                    })
                    .onFailure(ex->{
                        ex.printStackTrace();
                    });
    ...
    public void stop(@Observes ShutdownEvent event, Vertx vertx) {
        System.out.println("TicketLifecycle.stop()");
        ...
        consuClient.deregisterService(serviceId)
                .onSuccess(it->{
                    System.out.println("Ticket service deregistered in Consul");
                })
                .onFailure(ex->{
                    ex.printStackTrace();
                });
